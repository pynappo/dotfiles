...
var urgent_clients: { [key: string]: boolean } = {};
var urgent_workspaces: { [key: number]: number } = {};
hyprland.connect("urgent", (_, client) => {
  urgent_clients[client.address] = true;
  urgent_workspaces[client.workspace.id]++;
});
bind(hyprland, "focused_client").subscribe((client) => {
  if (urgent_clients[client.address]) {
    urgent_clients[client.address] = false;
    urgent_workspaces[client.workspace.id]--;
  }
});
hyprland.connect("client-removed", (_, client) => {
  delete urgent_clients[client];
});
hyprland.connect("workspace-removed", (_, id) => {
  delete urgent_workspaces[id];
});
function Workspaces({
  gdkmonitor,
                ...
                setup={(self) => {
                  disconnectors.push(
                    bind(hyprland, "focused_workspace").subscribe((focused) => {
                      const currentFocused = focused.id === ws.id;
                      if (currentFocused) {
                        self.toggleClassName("urgent", false);
                      }
                      console.log(`focused ${ws.id}: ${currentFocused}`);
                      self.toggleClassName("focused", currentFocused);
                    }),
                  );
                  self.hook(hyprland, "urgent", (_, urgent_client) => {
                    if (urgent_workspaces[ws.id] > 0) {
                      self.toggleClassName("urgent", true);
                    }
                    console.log(urgent_client);
                  });
                  disconnectors.push(
