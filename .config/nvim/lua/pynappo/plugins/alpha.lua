return {
  ...
  config = function(_, options)
    ...
    local function get_extension(fn) return vim.fn.fnamemodify(fn, ':e') end
    ...
    local function mru(start, dir, items_number)
      ...
      for i, filename in ipairs(oldfiles) do
        ...
        if #short_fn > options.width then
          ...
          if #short_fn > options.width then short_fn = plenary_path.new(short_fn):shorten(1, { -1 }) end
        end
    ...
    local section_mru = {
      ...
      val = {
        ...
        {
          type = 'group',
          val = function() return { mru(0, cwd, 10) } end,
          opts = {
            width = options.width,
            shrink_margin = true,
          },
